...
// ExportQuery represents the export MB params query.
type ExportQuery struct {
	IsActive *bool
}

// ExportResult represents the export MB params result.
type ExportResult struct {
	FileContent []byte
	FileName    string
}

// ExportHandler handles the ExportMbParams query.
type ExportHandler struct {
	repo mbparam.Repository
}

// NewExportHandler creates a new ExportHandler.
func NewExportHandler(repo mbparam.Repository) *ExportHandler {
...
// mbParamExportHeaders lists the export column headers in column order.
var mbParamExportHeaders = []string{
	"No", "Code", "Name", "Type", "Description", "Default Value", "Default Option",
	"Unit", "Display Order", "Active", "Created At", "Created By",
}
...
func setupParamExcelSheet(f *excelize.File, sheetName string) error {
	...
	for col, header := range mbParamExportHeaders {
	...
	lastCol, err := excelize.CoordinatesToCellName(len(mbParamExportHeaders), 1)
	if err != nil {
...
// writeMbParamRow writes a single param entity's fields into the given Excel row.
func writeMbParamRow(writer *excelWriter, row int, idx int, e *mbparam.Entity) {
	writer.setCellValue(fmt.Sprintf("A%d", row), idx+1)
	writer.setCellValue(fmt.Sprintf("B%d", row), e.Code())
	writer.setCellValue(fmt.Sprintf("C%d", row), e.Name())
	writer.setCellValue(fmt.Sprintf("D%d", row), e.Type())
	writer.setCellValue(fmt.Sprintf("E%d", row), e.Description())
	writer.setCellValue(fmt.Sprintf("F%d", row), e.DefaultValue())
	writer.setCellValue(fmt.Sprintf("G%d", row), e.DefaultOption())
	writer.setCellValue(fmt.Sprintf("H%d", row), e.Unit())
	writer.setCellValue(fmt.Sprintf("I%d", row), e.DisplayOrder())
	writer.setCellValue(fmt.Sprintf("J%d", row), e.IsActive())
	writer.setCellValue(fmt.Sprintf("K%d", row), e.CreatedAt())
	writer.setCellValue(fmt.Sprintf("L%d", row), e.CreatedBy())
}

// Handle executes the export MB params query.
func (h *ExportHandler) Handle(ctx context.Context, query ExportQuery) (result *ExportResult, err error) {
	params, err := h.repo.ListAll(ctx, mbparam.ExportFilter{IsActive: query.IsActive})
	if err != nil {
		return nil, fmt.Errorf("failed to get mb params for export: %w", err)
	}
	...
	for i, e := range params {
		writeMbParamRow(writer, i+2, i, e)
	}
	...
	writer.setColWidth("E", "E", 30)
	writer.setColWidth("F", "G", 15)
	writer.setColWidth("H", "J", 12)
	writer.setColWidth("K", "L", 20)
