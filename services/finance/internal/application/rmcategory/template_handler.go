...
// TemplateHandler handles the DownloadRMCategoryTemplate query.
...
func (h *TemplateHandler) Handle() (result *TemplateResult, err error) {
	...
	sheetName := "RM Category Import Template"
	...
	// Add sample data
	sampleData := [][]string{
		{"CHIP", "Chips", "Raw material chips for production"},
		{"OIL", "Oil", "Various types of oil"},
		{"DYES", "Dyes", "Coloring materials"},
		{"CHEM", "Chemicals", "Chemical compounds"},
	}
	...
	if _, sheetErr := f.NewSheet(notesSheet); sheetErr != nil {
		...
		notesWriter.setCellValue("A3", "1. Code: Unique, uppercase letters/numbers/underscores (e.g., CHIP, OIL_PALM)")
	...
	return &TemplateResult{
		...
		FileName:    "rm_category_import_template.xlsx",
	}, nil
}
