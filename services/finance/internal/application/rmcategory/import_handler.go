...
// ImportCommand represents the import RMCategories command.
...
// ImportResult represents the import RMCategories result.
...
// ImportHandler handles the ImportRMCategories command.
type ImportHandler struct {
	repo rmcategory.Repository
}

// NewImportHandler creates a new ImportHandler.
func NewImportHandler(repo rmcategory.Repository) *ImportHandler {
	return &ImportHandler{repo: repo}
}

// Handle executes the import RMCategories command.
...
func (h *ImportHandler) processRow(ctx context.Context, row []string, rowNum int32, cmd ImportCommand, result *ImportResult) {
	...
	// Validate fields
	code, err := h.validateRowData(data, rowNum, result)
	if err != nil {
...
func (h *ImportHandler) validateRowData(data rowData, rowNum int32, result *ImportResult) (rmcategory.Code, error) {
	// Validate code
	code, err := rmcategory.NewCode(data.code)
	if err != nil {
		result.FailedCount++
		result.Errors = append(result.Errors, ImportError{
			RowNumber: rowNum,
	...
	if data.name == "" {
		...
		return rmcategory.Code{}, fmt.Errorf("name cannot be empty")
	}
...
func (h *ImportHandler) handleDuplicate(ctx context.Context, code rmcategory.Code, data rowData, rowNum int32, cmd ImportCommand, result *ImportResult) {
...
// updateExisting updates an existing RMCategory.
func (h *ImportHandler) updateExisting(ctx context.Context, code rmcategory.Code, data rowData, rowNum int32, updatedBy string, result *ImportResult) {
...
// createCategory creates a new RMCategory.
func (h *ImportHandler) createCategory(ctx context.Context, code rmcategory.Code, data rowData, rowNum int32, createdBy string, result *ImportResult) {
	entity, err := rmcategory.NewRMCategory(code, data.name, data.description, createdBy)
	if err != nil {
		result.FailedCount++
		result.Errors = append(result.Errors, ImportError{
			RowNumber: rowNum,
