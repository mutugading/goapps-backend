...
func (p *Publisher) Publish(ctx context.Context, routingKey string, payload any) error {
	...
	// Inject the current span's trace context into the AMQP headers so the
	// downstream consumer (worker) can continue the trace. When tracing is
	// disabled the propagator is the global no-op and writes nothing.
	headers := amqp.Table{}
	otel.GetTextMapPropagator().Inject(ctx, propagation.TextMapCarrier(amqpHeaderCarrier(headers)))
	pub := amqp.Publishing{
